...
def classify_img_data(img_data):
        ...
        if success and len(req.results()) != 0  :
                ...
                '''
                r1_id       = str(req.results()[0].labels()[0].identifier())
                r1_conf     = req.results()[0].labels()[0].confidence()
                r1_ob_x     = req.results()[0].boundingBox().origin.x
                r1_ob_y     = req.results()[0].boundingBox().origin.y
                r1_ob_w     = req.results()[0].boundingBox().size.width
                r1_ob_h     = req.results()[0].boundingBox().size.height

                results = [[r1_id, r1_conf, r1_ob_x, r1_ob_y, r1_ob_w, r1_ob_h ]]

                if len(req.results()) > 1 :

                        r2_id       = str(req.results()[1].labels()[0].identifier())
                        r2_conf     = req.results()[1].labels()[0].confidence()
                        r2_ob_x     = req.results()[1].boundingBox().origin.x
                        r2_ob_y     = req.results()[1].boundingBox().origin.y
                        r2_ob_w     = req.results()[1].boundingBox().size.width
                        r2_ob_h     = req.results()[1].boundingBox().size.height

                        results.append([r2_id, r2_conf, r2_ob_x, r2_ob_y, r2_ob_w, r2_ob_h   ])

                if len(req.results()) > 2 :

                        r3_id       = str(req.results()[2].labels()[0].identifier())
                        r3_conf     = req.results()[2].labels()[0].confidence()
                        r3_ob_x     = req.results()[2].boundingBox().origin.x
                        r3_ob_y     = req.results()[2].boundingBox().origin.y
                        r3_ob_w     = req.results()[2].boundingBox().size.width
                        r3_ob_h     = req.results()[2].boundingBox().size.height

                        results.append([r3_id, r3_conf, r3_ob_x, r3_ob_y, r3_ob_w, r3_ob_h   ])
                '''
                #results = [[r1_id, r1_conf, r1_ob_x, r1_ob_y, r1_ob_w, r1_ob_h ]]
                #print(results)
...
#                ui.fill_rect(x2, y2, w2, h2)
                ...
                label_status.text = label_status.text + label + ':' + str(confidence) + '   '
...
def main():
    ...
    main_view.name = 'CoreML by objc_util YOLO'
